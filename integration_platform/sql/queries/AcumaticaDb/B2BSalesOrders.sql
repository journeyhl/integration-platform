...
select s.OrderType									OrderType
	 ...
	 , sa.PostalCode ShipZip
	 , ba.PostalCode BillZip
...
select OrderType
	 ...
	 , case when len(s.ShipZip) > 30 and s.OrderType = 'ZA' then null
	 		when len(s.ShipZip) > 30 then left(s.ShipZip, 30)
	   		else s.ShipZip 
	   end ShipZip
	 , case when len(s.BillZip) > 30 and s.OrderType = 'ZA' then null
	 		when len(s.BillZip) > 30 then left(s.BillZip, 30)
	   		else s.BillZip 
	   end BillZip
