...
class AcuToDbcCustomers(Pipeline):
    '''`AcuToDbcCustomers`(Pipeline)
    ---
    <hr>

    Gets all ____ and upserts to **acu.Customers**

    # Extraction
     - Gets all QT type Sales Orders from AcumaticaDb that were modified within the last day and loads them to **acu.Customers**

    # Transformation
     - Transforms extracted data into a format needed for **acu.Customers**

    # Load
     - Upsert to **acu.Customers** via :class:`~connectors.sql.SQLConnector`.:meth:`~connectors.sql.SQLConnector.checked_upsert_paginated`

    # Results Logging
     - None needed
    '''
    def __init__(self, function: str, env: str = 'prod'):
        super().__init__(pipeline_name='acu-to-dbc-customers', function=function, env=env)
    ...
    def extract(self) -> dict[str, pl.DataFrame]:
        acu_extract = self.acudb.query_to_dataframe(self.acudb.queries.AcuToDbc_Customers)
        # dbc_extract = self.centralstore.query_db('select distinct CustomerNbr from acu.Customers where LastChecked is not null')
        data_extract = {
    ...
    def transform(self, data_extract: dict[str, pl.DataFrame]):
        ...
        #     dbc_extract, on='CustomerNbr', how='anti'
        # )
